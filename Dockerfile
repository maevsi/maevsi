...
FROM node:19.3.0-slim AS development
...
# Should be the specific version of `node:slim`.
FROM node:19.3.0-slim AS prepare
...
FROM node:19.3.0-slim AS build
...
# Should be the specific version of `node:slim`.
# Could be the specific version of `node:alpine`, but the `prepare` stage uses slim too.
FROM node:19.3.0-slim AS test-unit
...
FROM cypress/included:12.2.0 AS test-integration_base
...
# Should be the specific version of `cypress/included`.
FROM cypress/included:12.2.0 AS test-integration
...
# Should be the specific version of `node:slim`.
FROM node:19.3.0-slim AS collect
