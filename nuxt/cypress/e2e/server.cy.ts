      ...
      cy.request({
        ...
        expect(resp.headers['content-security-policy']).to.equal(
          Cypress.env('NODE_ENV') === 'production'
            ? "connect-src 'self' blob: https://postgraphile.localhost:3000 https://tusd.localhost:3000 https://*.google-analytics.com https://*.analytics.google.com https://localhost:3000/cdn-cgi/rum;base-uri 'none';default-src 'none';font-src 'self';form-action 'self';frame-ancestors 'none';frame-src 'none';img-src 'self' blob: data: https://tusd.localhost:3000 https://*.google-analytics.com https://www.gravatar.com/avatar/;manifest-src 'self';media-src 'none';object-src 'none';prefetch-src 'self';report-uri https://dargmuesli.report-uri.com/r/d/csp/enforce;script-src blob: 'self' https://static.cloudflareinsights.com https://*.google-analytics.com https://www.googletagmanager.com/gtag/js 'unsafe-inline' 'unsafe-eval';style-src 'self' 'unsafe-inline';"
