...
BEGIN
  IF ("username" = '' AND "password" = '') THEN
    RETURN (SELECT (maevsi.uuid_generate_v1mc(), 'maevsi_anonymous', NULL, NULL, maevsi.invite_claim_array())::maevsi.jwt);
  ELSIF ("username" IS NOT NULL AND "password" IS NOT NULL) THEN
    RETURN (SELECT (maevsi.uuid_generate_v1mc(), 'maevsi_account', account.contact_id, account.username, NULL)::maevsi.jwt
