  ...
  test('sets the correct CSP header', async ({ request }) => {
    ...
    expect(resp.headers()['content-security-policy']).toEqual(
      process.env.NODE_ENV === 'production'
        ...
        : "connect-src 'self' blob: https://maev.si https://postgraphile.maev.si https://tusd.maev.si https://*.analytics.google.com https://*.google-analytics.com https://*.googletagmanager.com https://cdn.jsdelivr.net/npm/@sec-ant/ http://localhost:24678/_nuxt/ https://localhost:24678/_nuxt/ ws://localhost:24678/_nuxt/ wss://localhost:24678/_nuxt/;frame-src https://challenges.cloudflare.com 'self';base-uri 'none';default-src 'none';font-src 'self' data:;form-action 'self';frame-ancestors 'none';img-src 'self' blob: data: https://tusd.maev.si https://*.google-analytics.com https://*.googletagmanager.com https://www.gravatar.com/avatar/;manifest-src 'self';media-src 'none';object-src 'none';prefetch-src 'self';report-uri https://dargmuesli.report-uri.com/r/d/csp/enforce;script-src blob: 'self' https://challenges.cloudflare.com https://static.cloudflareinsights.com https://*.googletagmanager.com https://polyfill.io/v3/polyfill.min.js 'unsafe-inline' 'unsafe-eval';style-src 'self' 'unsafe-inline';",
    )
  })
})
